...
| display_columns             | `{}`                          | Boolean toggles for container columns (alternative to format). Set `containers.*` keys to enable/disable columns.            |
...
**Note:** The boolean configuration currently uses a standard field order (ID, Image, Command, Created, Status, Ports, Names) and cannot be reordered. For flexible column arrangements, use the template format above.
